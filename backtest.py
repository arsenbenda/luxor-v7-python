# ============================================================
# LUXOR V7 PRANA - BACKTEST MODULE v1.0.0
...
class LuxorBacktester:
    ...
    def run(self, df_historical: pd.DataFrame, 
        ...
        Args:
            ...
            lookback_days: Rolling window size for signal generation
        ...
        if len(df) < lookback_days + 50:
            raise ValueError(f"Insufficient data: need {lookback_days + 50}, have {len(df)}")
        ...
        for i in range(lookback_days, len(df)):
            ...
            try:
                    ...
                    if self.open_position is None:
                        ...
                        if conf_value >= min_conf_value and direction != "FLAT" and direction != "WAIT":
                            ...
                            self._enter_trade(
                                ...
                                stop_loss=setup.get("stop_loss", current_price * 0.95),
                                take_profit=setup.get("tp1", current_price * 1.05),
                ...
                logger.warning(f"[BACKTEST] Signal error at index {i}: {e}")
                continue
            ...
            # Progress logging
            if i % 100 == 0:
                logger.info(f"[BACKTEST] Progress: {i}/{len(df)} ({100*i/len(df):.1f}%)")
        ...
        logger.info(f"[BACKTEST] Complete: {result.total_trades} trades, {result.win_rate:.1f}% WR, {result.total_return_pct:.2f}% return")
    ...
    def _enter_trade(self, direction: str, entry_price: float, stop_loss: float,
        ...
        # Apply commission
        commission = actual_entry * commission_pct
        ...
        self.capital -= position_value * commission_pct
        
        logger.debug(f"[TRADE] Entered {direction} at ${actual_entry:.2f}, SL=${stop_loss:.2f}, TP=${take_profit:.2f}")
    ...
    def _close_position(self, exit_price: float, exit_date: str, reason: str,
        ...
        logger.debug(f"[TRADE] Closed {pos.direction} at ${actual_exit:.2f}, PnL=${pnl:.2f} ({pnl_pct*100:.2f}%), Reason={reason}")
    ...
    def _calculate_metrics(self, df: pd.DataFrame) -> BacktestResult:
        ...
        losses = [t.pnl for t in self.trades if t.status == "LOSS"]
    ...
    def print_summary(self, result: BacktestResult):
        ...
        print("\n" + "="*60)
        print("LUXOR V7 PRANA - BACKTEST RESULTS")
        print("="*60)
...
def run_backtest_example():
    ...
    # Load historical data (example - you need to provide actual data)
    # Option 1: From CSV
    # df = pd.read_csv("btc_historical.csv")
    
    # Option 2: Fetch live and use as historical
    try:
        df = system.fetch_real_binance_data(use_cache=False, symbol="BTCUSDT")
        print(f"Loaded {len(df)} candles")
    except Exception as e:
        print(f"Failed to fetch data: {e}")
        print("Please provide historical data manually")
        return
    ...
    result = backtester.run(
        df_historical=df,
        lookback_days=300,  # Use 300 days of data for each signal
